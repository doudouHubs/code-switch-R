...
const speechProviderId = ref(getCachedString('speechProviderId', ''))
const speechModelId = ref(getCachedString('speechModelId', ''))
const settingsLoading = ref(true)
const saveBusy = ref(false)
const modelInstructionsSaving = ref(false)
// 托盘预算配置已停止提供编辑入口，但旧配置仍属于持久化兼容字段。
// 保存其他设置时必须带回最近一次完整快照，避免缺省字段被 Go 结构体反序列化为零值。
let loadedSettingsSnapshot: AppSettings | null = null
...
const loadAppSettings = async () => {
  ...
  try {
    ...
    speechProviderId.value = speechSelection.speech_provider_id ?? ''
    speechModelId.value = speechSelection.speech_model_id
    ...
    localStorage.setItem('app-settings-speechModelId', speechModelId.value)
  } catch (error) {
    console.error('failed to load app settings', error)
    loadedSettingsSnapshot = null
...
const persistAppSettings = async () => {
  ...
  saveBusy.value = true
  try {
    // 空的 platform/provider 表示未配置；语音识别必须使用独立引用，不能回退到聊天或 TTS 配置。
    speechProviderPlatform.value = speechProviderPlatform.value.trim()
    speechProviderId.value = speechProviderId.value.trim()
    speechModelId.value = speechModelId.value.trim()
    const payload: AppSettings = {
      // 预算字段只作为兼容数据透传，不能在设置页保存时被隐式清零。
      ...(loadedSettingsSnapshot ?? await fetchAppSettings()),
    ...
    const payloadWithSpeech = saveSpeechProviderSelection(payload, {
      speech_provider_platform: speechProviderPlatform.value,
      speech_provider_id: speechProviderId.value,
      speech_model_id: speechModelId.value,
    })
    const savedSettings = await saveAppSettings(payloadWithSpeech)
    loadedSettingsSnapshot = savedSettings
    ...
    <div class="general-page">
      <section>
        ...
        <div class="mac-panel">
          ...
          <ListItem :label="$t('components.general.label.requestCapture')">
            <div class="toggle-with-hint">
              <label class="mac-switch">
                <input
                  type="checkbox"
                  :disabled="settingsLoading || saveBusy"
                  v-model="requestCaptureEnabled"
                  @change="persistAppSettings"
                />
                <span></span>
              </label>
          ...
          <ListItem :label="$t('components.general.label.speechProviderPlatform')">
            <input
              type="text"
              :disabled="settingsLoading || saveBusy"
              v-model="speechProviderPlatform"
              :placeholder="$t('components.general.label.speechProviderPlatformPlaceholder')"
              @change="persistAppSettings"
              class="mac-input capture-dir-input"
            />
          </ListItem>
          <ListItem :label="$t('components.general.label.speechProviderId')">
            <input
              type="text"
              :disabled="settingsLoading || saveBusy"
              v-model="speechProviderId"
              :placeholder="$t('components.general.label.speechProviderIdPlaceholder')"
              @change="persistAppSettings"
              class="mac-input capture-dir-input"
            />
          </ListItem>
          <ListItem :label="$t('components.general.label.speechModelId')">
            <div class="capture-dir-controls">
              <input
                type="text"
                :disabled="settingsLoading || saveBusy"
                v-model="speechModelId"
                :placeholder="$t('components.general.label.speechModelIdPlaceholder')"
                @change="persistAppSettings"
                class="mac-input capture-dir-input"
              />
              <span v-if="!speechProviderPlatform || !speechProviderId || !speechModelId" class="hint-text">
                {{ $t('components.general.label.speechProviderNotConfigured') }}
              </span>
            </div>
          </ListItem>
        </div>

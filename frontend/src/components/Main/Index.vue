  ...
  <div class="main-shell">
    ...
    >
      <form class="vendor-form" @submit.prevent="submitModal">
                ...
                <div v-if="activeTab !== 'gemini'" class="form-field">
                  <ModelCategoryEditor v-model="modalState.form.modelCategories" />
                </div>
...
interface GeminiProvider {
  ...
  // Wails map[string]string binding 使用可选索引值，保持本地兼容类型与生成契约一致。
...
const geminiToCard = (provider: GeminiProvider, index: number): AutomationCard => ({
  ...
  // Gemini 只有一个默认模型；沿用通用卡片的 map 形状，保证主页面编辑后
  // 再写回 Gemini 专属 modelCategory 时不会丢失能力声明。

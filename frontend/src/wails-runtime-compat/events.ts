...
import { isWailsRuntimeAvailable, newRuntimeCaller, objectNames } from "./runtime.js";
import { eventListeners, Listener, listenerOff } from "./listener.js";
import { connectBrowserEventStream } from "./browserBridge.js";
...
if (!isWailsRuntimeAvailable()) {
    void connectBrowserEventStream((name, data) => dispatchWailsEvent({ name, data })).catch(() => undefined);
}

export { Types } from "./event_types.js";

/**
 * A table of data types for all known custom events.
 */
export interface CustomEvents {
}

/**
 * Either a known custom event name or an arbitrary string.
 */
...
export type WailsEventData<E extends WailsEventName = WailsEventName> =
  E extends keyof CustomEvents ? CustomEvents[E] : any;
...
/**
 * Backward-compatible alias used by older generated bindings.
 */
export type Callback = WailsEventCallback;
...
export class WailsEvent<E extends WailsEventName = WailsEventName> {
    ...
    constructor(name: E, data: WailsEventData<E> = null as WailsEventData<E>) {
        this.name = name;
        this.data = data;
    }
}
...
export function OnMultiple<E extends WailsEventName = WailsEventName>(eventName: E, callback: WailsEventCallback<E>, maxCallbacks: number) {
    ...
    const thisListener = new Listener(eventName, callback as Callback, maxCallbacks);
 ...
 */
export function Emit<E extends WailsEventName = WailsEventName>(name: E, data?: WailsEventData<E>): Promise<void> {
    let eventName: string;
    let eventData: WailsEventData<E>;

    if (typeof name === 'object' && name !== null && 'name' in name && 'data' in name) {
        // If name is an object with a name property, use it directly
        eventName = name['name'] as string;
        eventData = name['data'] as WailsEventData<E>;
    } else {
        // Otherwise use the standard parameters
        eventName = name as string;
        eventData = data as WailsEventData<E>;
    }

    if (!isWailsRuntimeAvailable()) {
        dispatchWailsEvent({ name: eventName, data: eventData });

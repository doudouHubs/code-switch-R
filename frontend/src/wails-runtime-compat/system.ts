...
const _invoke = (function () {
    try {
        if (isWailsRuntimeAvailable() && (window as any).chrome?.webview?.postMessage) {
            return (window as any).chrome.webview.postMessage.bind((window as any).chrome.webview);
        } else if (isWailsRuntimeAvailable() && (window as any).webkit?.messageHandlers?.['external']?.postMessage) {

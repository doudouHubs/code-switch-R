...
func TestGeminiProviderDuplicatePreservesModelCategory(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	t.Setenv("USERPROFILE", tmpHome)
	if err := os.MkdirAll(filepath.Join(tmpHome, ".code-switch"), 0o755); err != nil {
		t.Fatalf("创建 Gemini 临时配置目录失败: %v", err)
	}

	service := NewGeminiService("127.0.0.1:18100")
	if err := service.AddProvider(GeminiProvider{
		ID:            "speech-source",
		Name:          "Speech Source",
		Model:         "whisper-custom",
		ModelCategory: "speech",
	}); err != nil {
		t.Fatalf("写入源 provider 失败: %v", err)
	}
	cloned, err := service.DuplicateProvider("speech-source")
	if err != nil {
		t.Fatalf("DuplicateProvider() error = %v", err)
	}
	if cloned.ModelCategory != "speech" {
		t.Fatalf("复制 provider 丢失模型类别: %#v", cloned)
	}

	// 重新加载，验证类别确实进入持久化文件，而不是只存在返回对象里。
	reloaded := NewGeminiService("127.0.0.1:18100")
	providers := reloaded.GetProviders()
	for _, provider := range providers {
		if provider.ID == cloned.ID && provider.ModelCategory != "speech" {
			t.Fatalf("持久化副本模型类别 = %q", provider.ModelCategory)
		}
	}
}

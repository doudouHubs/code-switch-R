...
type petWindowDriver interface {
	...
	SetPlatformLayer(platformID string) (bool, error)
	IsFocused() bool
}
...
// SetAlwaysOnTop 保留兼容调用；桌宠运行时的层级由 SetPlatformLayer 负责。
...
func (w *PetWindow) SetPlatformLayer(platformID string) error {
	...
	if err != nil {
		return fmt.Errorf("set pet window platform layer: %w", err)
	}
	w.alwaysOnTop = topmost
	return nil
}

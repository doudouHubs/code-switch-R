...
func TestToggleSkillEnabledWritesCodexConfigOverride(t *testing.T) {
	cwd, err := os.Getwd()
	if err != nil {
		t.Fatalf("getwd failed: %v", err)
	}
	...
	skillPath := filepath.Join(projectRoot, ".codex", "skills", "demo-skill")
	writeTestSkill(t, skillPath, "Demo Skill")

	ss := NewSkillService()
	if err := ss.ToggleSkillEnabled("demo-skill", skillPlatformCodex, skillLocationProject, false); err != nil {
		t.Fatalf("toggle skill enabled failed: %v", err)
	}
	...
	if !ok {
		t.Fatalf("missing skills table: %#v", raw)
	}
	var matched bool
	for _, item := range normalizeTomlArray(skillsTable["config"]) {
		...
		if normalizePathKey(path) == normalizePathKey(skillPath) {
			matched = true
			if enabled {
				t.Fatalf("Codex skill override should disable %s: %#v", skillPath, entry)
			}
		}
	}
	if !matched {
		t.Fatalf("missing Codex skill override for %s: %#v", skillPath, raw)
	}
}
